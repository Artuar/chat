...
export const Chat: React.FunctionComponent<IChat> = ({
 trade
  ...
  const { fetchMessages, sendMessage, deleteCurrentTrade } = useDispatchActions();
  const [ text, setText ] = useState('');

  useEffect(() => {
    document.body.addEventListener('keydown', keyboardClickHandler);
    return () => document.body.removeEventListener('keydown', keyboardClickHandler);
  });
  ...
  return (
    ...
    <header className={styles.header}>
        ...
        <div className={styles.counterparty}>
          ...
          {' / '}
          <span className={styles.dislikes}>-{trade.counterparty.dislikes}</span>
        </div>
      </header>
      ...
      <footer className={styles.footer}>
        <input value={text} className={styles.messageField} type="text" placeholder="Type your message..." onChange={changeInputHandler}/>
