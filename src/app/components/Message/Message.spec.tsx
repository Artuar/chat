...
describe("Message", () => {
  ...
  it("should render user photo correctly", () => {
    ...
    expect(
      component
        .find("[data-hook='message-user-photo']")
        .getDOMNode()
        .getAttribute("src")
    ).toBe(MESSAGE.userPhoto);
  });

  it("should render text correctly", () => {
    const { component } = mountMessageComponent();
    expect(
      component.find("[data-hook='message-text']").getDOMNode().textContent
    ).toBe(MESSAGE.text);
  });
  ...
  it("should render send time correctly", () => {
    const { component } = mountMessageComponent();
    expect(
      component.find("[data-hook='message-send-time']").getDOMNode().textContent
    ).toBe(MESSAGE.sendTime);
  });
  ...
  it("should render received message correctly", () => {
    ...
    expect(
      component
        .find("[data-hook='message']")
        .getDOMNode()
        .getAttribute("data-message-type")
    ).toBe(MessageType.Received);
  });
