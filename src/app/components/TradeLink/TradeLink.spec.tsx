...
describe("TradeLink", () => {
  ...
  it("should render name correctly for selling trade", () => {
    ...
    expect(component.find('[data-hook="trade-link-name"]').getDOMNode().textContent).toBe(`${TRADE.counterparty.name} is buying`);
  });
  ...
  it("should render name correctly for buying trade", () => {
    ...
    expect(component.find('[data-hook="trade-link-name"]').getDOMNode().textContent).toBe(`${TRADE.counterparty.name} is selling`);
  });

  it("should render seen notification state", () => {
    const { component } = mountTradeLinkComponent();
    expect(component.find("[data-hook='trade-link-notification']").getDOMNode().getAttribute('data-notification-unseen')).toBe("false");
  });

  it("should render unseen notification state", () => {
    const { component } = mountTradeLinkComponent({} , {hasUnseenMessages: true});
    expect(component.find("[data-hook='trade-link-notification']").getDOMNode().getAttribute('data-notification-unseen')).toBe("true");
  });

  it("should render card correctly", () => {
    const { component } = mountTradeLinkComponent();

    expect(component.find('[data-hook="trade-link-card"]').getDOMNode().textContent).toBe(TRADE.card);
  });

  it("should render amounts correctly", () => {
    const { component } = mountTradeLinkComponent();

    expect(component.find('[data-hook="trade-link-amounts"]').getDOMNode().textContent).toBe("100 USD (0.01012556 BTC)");
  });

  it("should render counterparty photo correctly", () => {
    const { component } = mountTradeLinkComponent();
    expect(component.find("[data-hook='trade-link-counterparty-photo']").getDOMNode().getAttribute('src')).toBe(TRADE.counterparty.photoUrl);
  });

  it("should render paid status correctly", () => {
    const { component } = mountTradeLinkComponent();
    expect(component.find("[data-hook='trade-link-trade-status']").getDOMNode().textContent).toBe('PAID');
  });
  ...
  it("should render unpaid status correctly", () => {
    ...
    expect(component.find("[data-hook='trade-link-trade-status']").getDOMNode().textContent).toBe('NOT PAID');
  });
});
