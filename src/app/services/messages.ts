...
export const fetchMessages = (userId: number, tradeId: number): Observable<Message[]> => {
  return from(getMessagesMock(userId, tradeId));
};

export const sendMessage = (userId: number, tradeId: number, text: string): Observable<Message> => {
