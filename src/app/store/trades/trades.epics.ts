...
const fetchTradesEpic: Epic<RootAction, TradesAction, RootState, Services> = (
  action$,
  state$,
  { fetchTradesService }
) => {
  return action$.pipe(
    ...
    switchMap(() =>
      fetchTradesService().pipe(map(actions.requestTrades.success))
    )
  );
};

const deleteTradeEpic: Epic<RootAction, TradesAction, RootState, Services> = (
  action$,
  state$,
  { deleteTradeService }
) => {
  ...
  return deleteTrade$.pipe(
    switchMap(([_, { id }]) =>
      deleteTradeService(id).pipe(map(actions.deleteTrades.success))
    )
  );
};

export const tradesEpic = combineEpics(fetchTradesEpic, deleteTradeEpic);
