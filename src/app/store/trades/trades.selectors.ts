...
export const currentTradeSelector = createSelector(
  ...
  (trades, currentTradeId): Trade | undefined => trades.find(({ id }) => id === currentTradeId),
);

export const formattedTradesSelector = createSelector(
  userIdSelector,
  tradesSelector,
  (userId, trades): FormattedTrade[] => trades.map(({
    seller,
    buyer,
    ...props
}) => {
...
export const formattedCurrentTradeSelector = createSelector(
  ...
  currentTradeIdSelector,
  (trades, currentTradeId): FormattedTrade | undefined => trades.find(({ id }) => id === currentTradeId),
);
