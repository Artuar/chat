    ...
    setTimeout(() => {
      ...
      window.localStorage.setItem(
        ...
        JSON.stringify({
          ...
          notifications: {
            ...notifications,
            [String(userId)]: userNotifications.filter(
              (id: number) => id !== tradeId
            ),
          },
        })
      );
      ...
      return resolve(
        messages.filter((message: Message) => message.tradeId === tradeId)
      );
    }, 10);
  }) as Promise<Message[]>;
};
...
export const sendMessageMock = (
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const { seller, buyer } = trades.find(
        (trade: Trade) => trade.id === tradeId
      );
